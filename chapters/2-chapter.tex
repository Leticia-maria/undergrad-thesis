...
\begin{equation}
\label{stos}
        \tikzmarknode{sto}{\highlight{red}{$\chi_{nlm}(\zeta, r)$}} = \tikzmarknode{norm}{\highlight{gray}{$N$}} \tikzmarknode{radial}{\highlight{magenta}{$r^{n-1} e^{-\zeta r}$}} \tikzmarknode{harmonic}{\highlight{blue}{$Y_l^m (\theta, \Phi)$}}
\end{equation}
\begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
    ...
    \path (norm.south) ++ (1.00,-1.5em) node[anchor=north west,color=gray!67] (scalep){\textit{$\displaystyle \bigg{[} \frac{(2 \zeta)^{n-1}}{(2n)!} \bigg{]}^{1/2}$}};
    \draw [color=gray!87](norm.south) |- ([xshift=1.00em,color=gray]scalep.north east);

    \path (radial.north) ++ (-2.00,1.5em) node[anchor=south east,color=magenta!67] (scalep){\textit{função radial}};
    \draw [color=magenta!87](radial.north) |- ([xshift=-2.00em,color=magenta]scalep.south west);
...
\begin{equation}
    \textcolor{blue!87}{i^{m + |m|}\bigg{[}\displaystyle \frac{(2l+1)(l+|m|)!}{2(l + |m|)!}\bigg{]}^{1/2} P^m_l (cos \; \theta) \phi_m(\Phi)}
\end{equation}
