...
\begin{equation}
    \label{cor}
    R_{opt} = \frac{ \tikzmarknode{single}{ \highlight{blue}{$R_{C-C}$}} + 2 \tikzmarknode{double}{ \highlight{red}{$R_{C=C}$}}}{3} = 1.397 \textbf{\AA}
\end{equation}
...
\end{equation}
\begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
    \path (homa.north) ++ (-2em,1.5em) node[anchor=south east,color=blue!67] (scalep){\textbf{índice geométrico}};
