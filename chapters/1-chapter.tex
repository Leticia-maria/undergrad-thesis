...
Do ponto de vista experimental, o primeiro sucesso do modelo foi explicar a razão pela qual o \textit{benzo[qr]nafto[2,1,8,7-fghi]pentaceno} (\textbf{1})
reagiu prontamente com anidrido maleico, enquanto seu isômero \textit{tribenzo[fg,ij,rst]pentafeno} (\textbf{2}) não foi reativo \autoref{fig:exampledd}. Comparando esses dois produtos, torna-se evidente que a estrutura do composto (\textbf{2}) é quimicamente inerte, pois contém 5 sextetos, enquanto que a estrutura (\textbf{1}) pode agir como um dieno em uma reação Diels-Alder \autocite{Clar1958}.

\begin{figure}[htb]
	\caption{\label{fig:exampledd}Representação das estruturas (\textbf{1}) benzo[qr]nafto[2,1,8,7-fghi]pentaceno e (\textbf{2}) tribenzo[fg,ij,rst]pentafeno, onde (\textbf{2}) é a estrutura de Clar.}
	\begin{center}
...
\begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
    ...
    \path (bondAverage.south) ++ (1,-2em) node[anchor=north west,color=red!67] (scalep){\textit{$\displaystyle \gls{Rav} = \frac{\displaystyle \sum_{i=1}^{n} R_i}{\displaystyle n}$}};
...
\begin{equation}
    \label{eq:3}
    \tikzmarknode{HOMA}{\highlight{red}{HOMA}} = 1 - \frac{\tikzmarknode{norm}{\highlight{blue}{$\alpha_{HOMA}$}}}{n} \sum_i^n (\gls{Ropt} - \gls{Ri})^2
\end{equation}
\begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
...
De acordo com a \autoref{eq:rHOMA}, os valores de $\gls{alphaRHOMA}$ variam de 57.21 (\ce{NO}), 93.52 (\ce{CN}), 94.09 (\ce{CS}), 118.91 (\ce{CP}), 130.33 (\ce{NN}), 157.38 (\ce{CO}), até 257.7 (\ce{CC}) \autocite{Krygowski2000, krygowski2001}.
...
\end{equation}
\begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
    \path (alphah.north) ++ (1,1.5em) node[anchor=south west,color=blue!67] (scalep){\textit{$\alpha_{rHOMA} = \displaystyle \frac{2}{(R_{opt} - R_s)^2 + (R_{opt} - R_d)^2}$}};
    \draw [color=blue!87](alphah.north) |- ([xshift=1em,color=blue]scalep.south east);
...
A fim de descrever corretamente a deslocalização dos elétrons $\pi$ em sistemas heteroatômicos cíclicos e acíclicos, o \gls{HOMED} considera os comprimentos de ligação \ce{CC}, \ce{CX} e \ce{XY} simples e dupla em várias moléculas pequenas que
poderiam ser empregadas como moléculas de referência. No procedimento \gls{HOMED}, os compostos saturados acíclicos como o etano, a metilamina e o metanol podem ser usadas para parametrizar os comprimentos de ligação \ce{C-C}, \ce{C-N}, e \ce{C-O}; e as moléculas insaturadas acíclicas como: eteno, metilimina e formaldeído podem ser aplicadas às ligações duplas \ce{C=C}, \ce{C=N} e \ce{C=O}, da mesma forma que foi proposto para o índice \gls{HOMA} original. 
