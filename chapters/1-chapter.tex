...
\label{eq:alphas}
    \begin{cases}
        \alpha = \displaystyle \frac{(2i + 1)}{(i+1)(R_{opt} - R_s)^2 + i(R_{opt} - R_d)^2} \\[0.50cm]
...
De um ponto de vista matemático, o método empregado ao índice \gls{HOMED} é o mesmo do \gls{HOMA} e do \gls{rHOMA}, sendo diferentes apenas os comprimentos de referência $R_s$, $R_d$, $R_{opt}$
e o fator $\alpha$. O índice \gls{HOMED} descreve o mesmo fenômeno -
