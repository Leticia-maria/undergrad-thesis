...
\begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
    ...
    \path (HOMA.north) ++ (-0.75,1.5em) node[anchor=south east,color=red!67] (scalep){\textit{(aromático perfeito) = 1}};
    \draw [color=red!87](HOMA.north) |- ([xshift=-0.75em,color=red]scalep.south west);
