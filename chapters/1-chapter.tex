...
\begin{figure}[htb]
	...
	\end{center}
	\fonte{Autor(a) (2022). Os números são do \href{https://www-periodicos-capes-gov-br.ez46.periodicos.capes.gov.br/index.php?}{\textit{Portal da Capes}}.}
\end{figure}
...
\begin{figure}[htb!]
	...
	\end{center}
	\fonte{Autor(a) (2022). Adaptado de Caramori \textit{et al} \autocite{Caramori2009}.}
\end{figure}
...
\begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
    ...
    \path (parama.north) ++ (-1, 1.5em) node[anchor=south east,color=red!67] (scalep){\textit{$\displaystyle \Delta \delta_1 \bar{V}^{2/3}$}};
    \draw [color=red!87](parama.north) |- ([xshift=-1.65em,color=red]scalep.south west);

    \path (paramb.north) ++ (1,1.5em) node[anchor=south west,color=red!67] (scalep){\textit{$\displaystyle \frac{1}{\displaystyle \sum_{i,j} |(\Delta (\delta_2)_i - (\delta_2)_j|}$}};
    \draw [color=red!87](paramb.north) |- ([xshift=1.65em,color=red]scalep.south east);
\end{tikzpicture}
...
\begin{table}[htb]
    ...
    \bottomrule
	\end{tabular}
	\fonte{Autor(a) (2022). Os valores foram retirados de Krygowski \textit{et al}\autocite{krygowski-2014}.}
\end{table}
...
\begin{figure}[htb]
	...
	\end{center}
	\fonte{Autor(a) (2022). Os valores foram comparados com aqueles obtidos por Fringuelli \textit{et al}\autocite{Fringuelli1974}.}
\end{figure}
...
\begin{equation}
    \label{eq:2}
    \gls{Ropt} = \frac{ \tikzmarknode{single}{ \highlight{blue}{$R_s$}} + 2 \tikzmarknode{double}{ \highlight{red}{$R_d$}}}{3} = 1.40 \textit{\AA}
\end{equation}
\begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
    \path (single.north) ++ (-1,1.5em) node[anchor=south east,color=blue!67] (scalep){\textit{$R_s = 1.53$} \AA};
    \draw [color=blue!87](single.north) |- ([xshift=-3em,color=blue]scalep.south west);
    
    \path (double.north) ++ (1,1.5em) node[anchor=south west, color=red!67] (scalep){\textit{$R_d = 1.34$} \AA};
...
O valor obtido pela \autoref{eq:2} apresenta uma boa concordância com o valor experimental obtido para o benzeno (1.40 \AA) \footnote{Esse valor foi medido através de difração de raio X a 10 K\autocite{Stewart1991}.}. A fórmula para o \gls{HOMA} está abaixo:

\begin{figure}[htb]
    \vspace{2\baselineskip}
\begin{equation}
    \label{eq:3}
    \tikzmarknode{HOMA}{\highlight{red}{\textit{HOMA}}} = 1 - \frac{\tikzmarknode{norm}{\highlight{blue}{$\alpha_{HOMA}$}}}{n} \sum_i^n (\gls{Ropt} - \gls{Ri})^2
\end{equation}
\begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
    \path (norm.north) ++ (1,1.5em) node[anchor=south west,color=blue!67] (scalep){\textit{$\alpha_{HOMA} = 98.89$}};
    \draw [color=blue!87](norm.north) |- ([xshift=3em,color=blue]scalep.south east);
    
    \path (HOMA.north) ++ (-0.70,1.5em) node[anchor=south east,color=red!67] (scalep){\textit{(aromático) = 1}};
    \draw [color=red!87](HOMA.north) |- ([xshift=-0.50em,color=red]scalep.south west);
    
    \path (HOMA.south) ++ (-1,-1.5em) node[anchor=north east,color=red!67] (scalep){\textit{(não aromático) = 0}};
    \draw [color=red!87](HOMA.south) |- ([xshift=-0.49em,color=blue]scalep.north west);
\end{tikzpicture}
...
\end{equation}
\begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
    \path (en.north) ++ (1,1.5em) node[anchor=south west,color=blue!67] (scalep){\textit{$\alpha_{HOMA}(R_{opt} - R_{av})^2$}};
    \draw [color=blue!87](en.north) |- ([xshift=3em,color=blue]scalep.south east);
    
    \path (geo.south) ++ (-1,-1.5em) node[anchor=north east,color=red!67] (scalep){\textit{$\displaystyle \frac{1}{n} \sum_i \alpha_{HOMA}(R_{av} - R_i)^2$}};
...
\end{equation}
\begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
    \path (alphah.north) ++ (-0.3,1.5em) node[anchor=south east,color=blue!67] (scalep){\textit{$\alpha_{rHOMA} = \displaystyle \frac{2}{(R_{opt} - R_s)^2 + (R_{opt} - R_d)^2}$}};
    \draw [color=blue!87](alphah.north) |- ([xshift=-0.3em,color=blue]scalep.south west);
...
A fim de descrever corretamente a deslocalização dos elétrons $\pi$ em sistemas heteroatômicos cíclicos e acíclicos, o \gls{HOMED} considera os comprimentos de ligação \ce{CC}, \ce{CX} e \ce{XY} (em que X e Y são heteroátomos) simples e dupla em várias moléculas pequenas que
poderiam ser empregadas como referência para o $R_{opt}$. No procedimento \gls{HOMED}, os compostos saturados acíclicos como o etano, a metilamina e o metanol podem ser usados para parametrizar os comprimentos de ligação \ce{C-C}, \ce{C-N}, e \ce{C-O}; e as moléculas insaturadas acíclicas como: eteno, metilimina e formaldeído podem ser aplicadas às ligações duplas \ce{C=C}, \ce{C=N} e \ce{C=O}.  A escolha dos compostos mais simples como referência possibilita
...
\section{Multidimensionalidade da aromaticidade}
