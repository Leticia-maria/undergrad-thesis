...
Se dividirmos os elementos da matriz representada na \autoref{ap:eq:13c} por $\beta$, obtemos a \autoref{ap:eq:13d}.

\begin{equation}
\label{ap:eq:13d}
\begin{bmatrix}
    \displaystyle \frac{(\alpha - E)}{\beta} & 1 \\
    1  & \displaystyle \frac{(\alpha - E)}{\beta}
\end{bmatrix}
\end{equation}

Ao reduzir a expressão a $\displaystyle x = \frac{(\alpha - E)}{\beta}$, a matriz se transforma na \autoref{ap:eq:13e}.
...
Ou seja, os resultados da teoria de Hueckel mostram alguns aspectos para hidrocarbonetos conjugados, mantendo as ligações simples e duplas alternadas. 
