...
\end{equation}
\begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
    \path (alpha.north) ++ (1.2em,1.5em) node[anchor=south west,color=blue!67] (scalep){\textbf{integral de Coulomb}};
    \draw [color=blue!87](alpha.north) |- ([xshift=3em,color=blue]scalep.south east);
\end{tikzpicture}
\end{figure}
...
\begin{equation}
\label{ap:eq:11}
    \displaystyle \int (p_1 p_2) d\tau = \highlight{blue}{$S_{12}$} = \displaystyle \int (p_2 p_1) d\tau = \tikzmarknode{s21}{\highlight{blue}{$S_{21}$}}
\end{equation}
\begin{tikzpicture}[overlay,remember picture,>=stealth,nodes={align=left,inner ysep=1pt},<-]
    \path (s21.north) ++ (-1.2em,1.5em) node[anchor=south east,color=blue!67] (scalep){\textbf{integral de sobreposição}};
    \draw [color=blue!87](s21.north) |- ([xshift=-3em,color=blue]scalep.south west);
\end{tikzpicture}
\end{figure}
